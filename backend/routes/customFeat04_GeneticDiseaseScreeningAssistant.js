...
// Rare variant detected; AI searches literature, suggests counseling points, flags if family history relevant
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / cf-genetic-disease-screening-assistant',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_genetic_disease_screening_assistant`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-genetic-disease-screening-assistant feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Genetic disease screening assistant'.
Context: Rare variant detected; AI searches literature, suggests counseling points, flags if family history relevant
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-genetic-disease-screening-assistant/run error:', err);
