...
// Gap Feature: Embryos without exposed \'/embryo
// Embryos without exposed /embryo-scoring-ai (combined grading: morphology, genetic risk)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / gap-embryos-without-exposed-embryo',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Embryos without exposed \'/embryo'.\nProject: AIpoweredembryo.\nContext: Embryos without exposed /embryo-scoring-ai (combined grading: morphology, genetic risk)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['embryos-without-exposed-embryo', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'embryos-without-exposed-embryo', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['embryos-without-exposed-embryo']
    );
