...
// Gap Feature: AI route stubs (ai
// AI route stubs (ai-reports, ai-scores, predictions) need real model integration
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / gap-ai-route-stubs-ai',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'AI route stubs (ai'.\nProject: AIpoweredembryo.\nContext: AI route stubs (ai-reports, ai-scores, predictions) need real model integration\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['ai-route-stubs-ai', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'ai-route-stubs-ai', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
