...
// Gap Feature: Limited regulatory compliance tracking depth (CAP, CLIA)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / gap-limited-regulatory-compliance-tracking-depth-cap-c',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited regulatory compliance tracking depth (CAP, CLIA)'.\nProject: AIpoweredembryo.\nContext: Limited regulatory compliance tracking depth (CAP, CLIA)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-regulatory-compliance-tracking-depth-cap-c', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-regulatory-compliance-tracking-depth-cap-c', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-regulatory-compliance-tracking-depth-cap-c']
    );
