...
// Microscope images analyzed; AI grades morphology (blastocyst expansion, cell fragmentation, symmetry); compares against lab\'s manual grading
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / cf-computer-vision-embryo-grading',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_embryo_grading`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-computer-vision-embryo-grading feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision embryo grading'.
Context: Microscope images analyzed; AI grades morphology (blastocyst expansion, cell fragmentation, symmetry); compares against lab\'s manual grading
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-embryo-grading/run error:', err);
