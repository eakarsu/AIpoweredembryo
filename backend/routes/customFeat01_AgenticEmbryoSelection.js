...
// Multi-modal scoring (morphology images, genetic test results, age/health factors, preferences); AI recommends best embryo transfer order; generates counseling summary
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / cf-agentic-embryo-selection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_embryo_selection`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-embryo-selection feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic embryo selection'.
Context: Multi-modal scoring (morphology images, genetic test results, age/health factors, preferences); AI recommends best embryo transfer order; generates counseling summary
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-embryo-selection/run error:', err);
