...
// Patient ovarian reserve, age, prior responses; AI recommends medication protocols most likely to succeed
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / cf-cycle-protocol-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_cycle_protocol_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-cycle-protocol-optimization feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Cycle protocol optimization'.
Context: Patient ovarian reserve, age, prior responses; AI recommends medication protocols most likely to succeed
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-cycle-protocol-optimization/run error:', err);
