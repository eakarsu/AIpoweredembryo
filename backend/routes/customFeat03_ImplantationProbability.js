...
// Ensemble ML model combines embryo quality, genetic status, patient age/BMI, endometrial health; provides realistic success probability
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredembryo / cf-implantation-probability',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_implantation_probability`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-implantation-probability feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Implantation probability'.
Context: Ensemble ML model combines embryo quality, genetic status, patient age/BMI, endometrial health; provides realistic success probability
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-implantation-probability/run error:', err);
